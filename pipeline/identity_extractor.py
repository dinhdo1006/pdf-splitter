...
def extract_member_identity_from_signals(
    ...
    *,
    ...
    def _fill_cap_uy_if_missing(m: MemberIdentity) -> MemberIdentity:
        if m.m1 is None:
            try:
                ...
                if cu:
                    ...
                    m.m4 = cu.m4
                    m.m5 = cu.m5
                    m.sources.append(f"m_codes:cap_uy_catalog@{cu.ten_chinh}")
    ...
    for pn, sig in sorted(signals.items(), key=lambda kv: _rank(kv[0], kv[1])):
        ...
        if merged.has_member_folder_keys:
            merged = _fill_cap_uy_if_missing(merged)
            logger.info(
                f"[identity] extracted ho_ten={merged.ho_ten!r} "
                f"cccd={merged.cccd!r} tdv={merged.tdv!r} "
                f"folder_id={merged.folder_id!r} m_codes={merged.m1}.{merged.m2}.{merged.m3}.{merged.m4}.{merged.m5} "
